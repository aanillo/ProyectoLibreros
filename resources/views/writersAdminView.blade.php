...
<body class="flex flex-col min-h-screen text-white font-[Brawler]">
    ...
    <main class="flex-grow flex flex-col items-center bg-white text-black px-6 mt-48">
        ...
        <div class="flex-grow flex flex-row items-center gap-4 mb-8">
            ...
            <a href="{{ route('insertWriter') }}" class="btnHome bg-amber-200 text-[#322411] font-bold border-2 border-solid border-black px-10 py-1.5 rounded-md hover:bg-yellow-600">Insertar</a>
        </div>
    ...
    x-data="{
        ...
        get filteredWriters() {
            ...
            return this.writers.filter(writer =>
                writer.nombre.toLowerCase().includes(this.searchQuery.toLowerCase())  // Usa nombre en lugar de titulo
            );
        },
        get paginatedWriters() {
            const start = (this.currentPage - 1) * this.writersPerPage;
            return this.filteredWriters.slice(start, start + this.writersPerPage);
        },
        get totalPages() {
            return Math.max(1, Math.ceil(this.filteredWriters.length / this.writersPerPage));
        },
        writers: @js($writers),  
        baseUrl: '{{ url('writers/admin') }}'
    }">
    ...
    <table class="min-w-full bg-white border border-gray-200">
        <thead>
            <tr class="bg-[#322411] text-left text-lg text-white">
                ...
                <th class="px-4 py-2">Fecha fallecimiento</th>
        ...
        <tbody>
            <template x-for="writer in paginatedWriters" :key="writer.id">
                <tr class="border-b text-black text-lg">
                    ...
                    <td class="px-4 py-2 max-w-[350px]" x-text="writer.nombre_completo"></td>
                    <td class="px-4 py-2" x-text="writer.pais"></td>
                    <td class="px-4 py-2" x-text="writer.nacimiento"></td>
                    <td class="px-4 py-2" x-text="writer.fallecimiento"></td>
                    <td class="px-4 py-2">
                        <a :href="baseUrl + '/writers/' + writer.id + '/edit'" class="text-blue-500 hover:underline font-bold">Editar</a>
                        |
                        <form :action="baseUrl + '/writers/' + writer.id" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-500 hover:underline font-bold" onclick="return confirm('¿Estás seguro de que deseas eliminar este autor?')">Eliminar</button>
                        </form>
                    </td>
                </tr>
            </template>
